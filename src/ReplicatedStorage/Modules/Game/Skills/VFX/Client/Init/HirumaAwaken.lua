...
local function NotifyServerCutsceneEnded(ServerToken: number): ()
	...
	if Packets and Packets.PerfectPassCutsceneEnded and Packets.PerfectPassCutsceneEnded.Fire then
		Packets.PerfectPassCutsceneEnded:Fire(ServerToken)
	end
end
...
function TryCompleteCutscene(Token: number): ()
	...
	if State.ServerToken ~= nil then
		NotifyServerCutsceneEnded(State.ServerToken)
	end
	Cleanup()
end
...
local function ResolveCharsFolder(): Instance?
	...
	return CutsceneRigUtils.FindChildByAliases(AssetsFolder, { CHARS_FOLDER_NAME }, true)
end

local function GetFallbackModels(): {Model}
	if CachedFallbackModels then
...
local function BuildFallbackModelPool(): {Model}
	local Pool: {Model} = {}
	for _, Model in GetFallbackModels() do
...
local function TakeRandomFallbackModel(Pool: {Model}): Model?
	if #Pool <= 0 then
		return nil
	end

	local Index: number = math.random(1, #Pool)
	local Model: Model = Pool[Index]
...
local function ApplyFallbackAppearance(ActorModel: Model, FallbackModel: Model?, TeamNumber: number?): ()
...
local function ApplyAppearancePlan(ActorModel: Model, Plan: ActorAppearancePlan?): ()
	...
	if SourcePlayer and ApplyPlayerAppearance(SourcePlayer, ActorModel) then
		return
	end

	ApplyFallbackAppearance(ActorModel, Plan.FallbackModel, Plan.TeamNumber)
end
...
local function BuildActorAppearancePlans(Payload: CutscenePayload): {[string]: ActorAppearancePlan}
	local FallbackPool: {Model} = BuildFallbackModelPool()
	...
	local function BuildPlan(SourcePlayerForPlan: Player?, TeamNumber: number?): ActorAppearancePlan
		...
		return {
			...
			FallbackModel = if ActualPlayer then nil else TakeRandomFallbackModel(FallbackPool),
	...
	return {
		...
		["Player 10"] = BuildPlan(SourcePlayer, SourceTeamNumber),
		["Player 7"] = BuildPlan(ReceiverPlayer, SourceTeamNumber),
	}
end
...
function Cleanup(): ()
	...
	if PendingServerToken ~= nil and not PendingServerCompletionSent then
		NotifyServerCutsceneEnded(PendingServerToken)
	end
...
local function StartCutscene(Payload: CutscenePayload): ()
	...
	local PendingAnimations: {{Target: Instance, AnimationId: string, IsRugbyBall: boolean?}} = {}
