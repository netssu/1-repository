...
local function IsNoInternetErrorMessage(error_message)
	local message = tostring(error_message)
	return string.find(message, "ConnectFail", 1, true) ~= nil
end

local function IsNoAccessErrorMessage(error_message)
	local message = tostring(error_message)
	return string.find(message, "403", 1, true) ~= nil
		or string.find(message, "Access forbidden", 1, true) ~= nil
		or string.find(message, "AccessForbidden", 1, true) ~= nil
		or string.find(message, "must publish", 1, true) ~= nil
end

local function ShouldUseLiveDataStoresInStudio()
	if IsStudio ~= true then
		return true
	end

	return game:GetAttribute(STUDIO_LIVE_DATASTORES_ATTRIBUTE) == true
end

local function SetDataStoreStateValue(new_state, error_message)
	if DataStoreState == new_state and (ProfileStore == nil or ProfileStore.DataStoreState == new_state) then
		return
	end

	local previous_state = DataStoreState
	DataStoreState = new_state

	if ProfileStore ~= nil then
		ProfileStore.DataStoreState = new_state

		if new_state ~= "Access" then
			table.clear(IssueQueue)

			if ProfileStore.IsCriticalState == true then
	...
	print(`[{script.Name}]: DataStore state changed from {previous_state} to {new_state}`)

	if error_message ~= nil and new_state ~= "Access" then
		warn(`[{script.Name}]: Falling back to {new_state} mode after DataStore access failure: {tostring(error_message)}`)
	end
end

local function RegisterError(error_message, store_name, profile_key) -- Called when a DataStore API call errors
	local error_message_string = tostring(error_message)
	warn(`[{script.Name}]: DataStore API error (STORE:{store_name}; KEY:{profile_key}) - {error_message_string}`)

	if IsStudio == true then
		if IsNoInternetErrorMessage(error_message_string) == true then
			SetDataStoreStateValue("NoInternet", error_message_string)
		elseif IsNoAccessErrorMessage(error_message_string) == true then
			SetDataStoreStateValue("NoAccess", error_message_string)
		end
	end

	table.insert(IssueQueue, os.clock()) -- Adding issue time to queue
	OnError:Fire(error_message_string, store_name, profile_key)
end
...
if IsStudio == true then
	...
	task.spawn(function()
		if ShouldUseLiveDataStoresInStudio() ~= true then
			print(`[{script.Name}]: Studio live DataStores disabled - set game attribute "{STUDIO_LIVE_DATASTORES_ATTRIBUTE}" to true to enable real save/load`)
			SetDataStoreStateValue("NoAccess", `Studio live DataStores disabled ({STUDIO_LIVE_DATASTORES_ATTRIBUTE} ~= true)`)
			return
		end
		...
		local no_internet_access = status == false and IsNoInternetErrorMessage(message)
		...
		if status == false and
			(IsNoAccessErrorMessage(message) == true or -- Cannot write to DataStore from studio if API access is not enabled
		...
		SetDataStoreStateValue(new_state)

	end)

else
	
	SetDataStoreStateValue("Access")
