...
local api = shared.fx
local parent = api.temp
local pool = api.util.pool
...
local partpool = pool.new(BoltPart, 20)
...
function LightningBolt.new(Attachment0, Attachment1, PartCount, ParentContainer, HasHighlight)
	local self = setmetatable({}, LightningBolt)
	PartCount = math.max(1, PartCount or 20)
	self._Container = (typeof(ParentContainer) == "Instance") and ParentContainer or api.temp
	if HasHighlight ~= nil then
		self._HasHighlight = HasHighlight
	else
		self._HasHighlight = self._Container:FindFirstChildOfClass("Highlight") ~= nil
	end
	...
	self.Speed = 7 --Governs how fast the bolt oscillates (i.e. how fast the fluctuating wave travels along bolt)
	...
	self.Shape = Enum.PartType.Cylinder --The shape of the bolt parts

	--Bolt Kinetic Properties

	--[[
		Allows for fading in (or out) of the bolt with time. Can also create a "projectile" bolt
		Recommend setting Speed to 0 if used as projectile (for better aesthetics)
	...
	for i = 1, PartCount do
		...
		-- Cycle through dead parts in the pool if any exist
		while part and not pcall(function() part.Parent = self._Container end) do
			part = partpool:take()
		end
...
function LightningBolt:_UpdateGeometry(
	...
	if self._HasHighlight and trans < 1 then
		trans = math.min(trans, 0.01)
	end
	...
	if Opacity > contractf then
		...
		BPart.Transparency = trans
		BPart.Shape = self.Shape
...
RunService.Heartbeat:Connect(function()
	for _, ThisBranch in pairs(ActiveBranches) do
		if not ThisBranch._Container or not ThisBranch._Container.Parent then
			ThisBranch:Destroy()
			continue
		end
