...
BoltAdorn.Image = "http://www.roblox.com/asset/?id=4955566540"
BoltAdorn.Transparency = 1
BoltAdorn.Color3 = Color3.new(1, 1, 1)
BoltAdorn.ZIndex = 0
BoltAdorn.Size = Vector2.new(0, 0)

local imagepool = pool.new(BoltAdorn, 20)
...
function LightningBolt.new(Attachment0, Attachment1, PartCount)
	...
	ScreenGui = guiParent:FindFirstChild("LightningBeams")
	...
	local api = shared.fx
	DummyPart = api.temp:FindFirstChild("AdorneePart")
	if not DummyPart then
		...
		DummyPart.Size = Vector3.new(0.001, 0.001, 0.001)
		DummyPart.Transparency = 1
		api.debris(DummyPart)
	...
	local MainBranchN = math.max(1, PartCount or 20)
...
game:GetService("RunService").Heartbeat:Connect(function ()
	...
	for _, ThisBranch in pairs(ActiveBranches) do
		if not DummyPart or not DummyPart.Parent then
			ThisBranch:Destroy()
			continue
		end
		if ThisBranch.Enabled == true then
			...
			if timePassed < (PulseLength + 1) / PulseSpeed then
				...
				for i = 1, PartsN do
					--local spd = NoiseBetween(i/PartsN, 1.5, 0.1*i/PartsN, -MinSpeed, MaxSpeed) --Can enable to have an alternative animation which doesn't shift the noisy lightning "Texture" along the bolt
					local BPart = Parts[i]
					local t1 = i/PartsN
