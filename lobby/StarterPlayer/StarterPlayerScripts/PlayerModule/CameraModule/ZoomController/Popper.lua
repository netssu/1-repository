...
local blacklist = {} do
	...
	local function ignoreAdded(child)
		charMap[child] = child
		refreshIgnoreList()
	end
	
	local function ignoreRemoved(child)
		charMap[child] = nil
		refreshIgnoreList()
	end
	...
	if workspace:FindFirstChild("Ignore") then
		workspace.Ignore.ChildAdded:Connect(ignoreAdded)
		workspace.Ignore.ChildRemoved:Connect(ignoreRemoved)
	else
		local waitConnection; waitConnection = workspace.DescendantAdded:Connect(function()
			
			if not workspace:FindFirstChild("Ignore") then return end
