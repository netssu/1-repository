...
function Effect.ShineEffect(tblinfo)
	if true then return end -- disabled by ace for now
	...
	if grad and Parent then
		...
		while Shine ~= nil and grad ~= nil and Border ~= nil do
			...
			UpdSpeed = UpdSpeed + RunService.RenderStepped:Wait();			
		end;
	end;
	Shine.Visible = false;
	task.wait((math.random(20, 45) * 0.1 + 1) / 2);
end
...
function Effect.StartRainbow(Gradient)
	...
	--	end;
	--end)();
	-- disabled by ace for performance
end
local function createOptimizedRotation(Object) -- optimised by yours truly, claude (Ace in chinese)
	-- Pre-calculate constants
	local ROTATION_SPEED = math.rad(90) -- 360/4 = 90 degrees per second

	-- Cache parent check to avoid expensive property access
	local lastParent = Object.Parent
	local parentCheckCounter = 0
	local PARENT_CHECK_INTERVAL = 10 -- Check parent every 10 frames

	local connection
	connection = RunService.Heartbeat:Connect(function(deltaTime)
		-- Optimized parent check (only every 10 frames)
		parentCheckCounter = parentCheckCounter + 1
		if parentCheckCounter >= PARENT_CHECK_INTERVAL then
		...
		-- Direct CFrame multiplication (most optimized)
		Object.CFrame = Object.CFrame * CFrame.new(0, 0, 0, 0, math.sin(deltaTime * ROTATION_SPEED * 0.5), 0, 0, math.cos(deltaTime * ROTATION_SPEED * 0.5), 0)
	end)
...
function Effect.SpinObject(Object)
	createOptimizedRotation(Object)
	--local Loop = nil;
