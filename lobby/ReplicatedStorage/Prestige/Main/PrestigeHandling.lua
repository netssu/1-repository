...
function Prestige.Reset(character, level, prestige)
	...
	local Billboard = character:FindFirstChild("Head"):FindFirstChild("Nametag")
	local LevelText = Billboard:FindFirstChild("LevelFrame"):FindFirstChild("LevelLabel")
	LevelText.Text = level
	local Prestige = Billboard:FindFirstChild("LevelFrame"):FindFirstChild("LevelLabel"):FindFirstChild("Prestige")
	if Prestige.Visible == false then
		Prestige.Visible = true
	end
	local ImageID = ImageModule[prestige]
	Prestige.Image = ImageID
end
